...
export function useAtomAgent(host: AtomAgentHost) {
  ...
  const startVoice=useCallback(async()=>{
    ...
    try{
      ...
      // iOS Safari only plays a WebRTC remote stream from an <audio> element
      // that is in the DOM, marked playsinline, and started with an explicit
      // play() inside the voice-start user gesture — a detached autoplay
      // element (fine in Electron) stays silent on mobile.
      const audio=document.createElement('audio');audio.autoplay=true;audio.setAttribute('playsinline','');audio.muted=speakerMutedRef.current;audio.style.display='none';document.body.appendChild(audio);remoteAudio.current=audio;realtimeConnection.ontrack=event=>{if(!voiceSessionIsActive(generation,realtimeConnection))return;const remoteStream=event.streams[0];if(event.track.kind!=='audio'||!remoteStream){append('system','Realtime voice response did not include exactly one audio stream','failed');stopVoice('error',generation);return;}remoteAudioTrackIds.current.add(event.track.id);if(remoteAudioTrackIds.current.size!==1){append('system','Realtime voice returned more than one remote audio track','failed');stopVoice('error',generation);return;}const current=audio.srcObject as MediaStream|null;if(current&&current.id!==remoteStream.id){append('system','Realtime voice returned more than one remote audio stream','failed');stopVoice('error',generation);return;}if(!current){audio.srcObject=remoteStream;void audio.play().catch(()=>{});console.info('[Atom Realtime] single remote audio playback path attached',{streamId:remoteStream.id,trackId:event.track.id});}};
