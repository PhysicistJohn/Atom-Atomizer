...
describe('released v3 open-set gate with DACS v7 refinement', () => {
  ...
  beforeAll(() => {
    ...
    vi.stubGlobal('fetch', (async (input) => {
      ...
      const root = url.pathname.startsWith('/classifier/v7/') ? v7Root : v3Root;
  ...
  it('does not load or run the closed-set DACS model after a v3 rejection', async () => {
    const capture = prefix(parityRow('causal-prefix-gated-N32768'), 20_000);
  ...
  it('runs DACS only after v3 accepts a known exact-rate capture', async () => {
  ...
  it('keeps v3 as the visible classifier outside the exact training rate', async () => {
    ...
    expect(result.runtime?.model).toBe('time-domain-v3');
    expect(result).toEqual(legacyResult);
    expect(requested.slice(requestsBefore)).toEqual([]);
  });
});
