...
class RendererErrorBoundary extends React.Component<React.PropsWithChildren, { error?: Error }> {
  ...
  override componentDidCatch(error: Error, info: React.ErrorInfo): void { console.error('[TinySA Atomizer] renderer failed loudly', error, info); }
  override render(): React.ReactNode {
    ...
    return <main className="renderer-fatal" role="alert">
      ...
      <h1>TinySA Atomizer could not start</h1>
      <p>{this.state.error.message}</p>
      <button onClick={() => { for (const key of Object.keys(localStorage)) if (key.startsWith('tinysa-atomizer:v2:')) localStorage.removeItem(key); location.reload(); }}>Reset local UI preferences and reload</button>
    </main>;
  }
}
