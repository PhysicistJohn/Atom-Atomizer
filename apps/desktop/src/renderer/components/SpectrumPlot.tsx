...
export function SpectrumPlot({ sweep, traces = [], markers = [], activeMarkerId, detections = [], display = DEFAULT_DISPLAY, busy, onMarkerPlace }: SpectrumPlotProps) {
  ...
  return <section className="plot-panel" aria-label="Spectrum plot">
    <div className="panel-header"><div><span className="live-indicator"/><strong>Live spectrum</strong><small>{visibleTraces.map((trace) => `T${trace.traceId} ${traceAbbreviation(trace.mode)}`).join(' · ') || 'AWAITING TRACE'}</small></div><div className="plot-meta"><span><Activity size={13}/>{sweep ? `${sweep.frequencyHz.length} bins` : 'Awaiting trace'}</span><span><Orbit size={13}/>{detections.length} signals</span><span>{visibleTraces.length} traces</span></div></div>
    <div className={`plot-canvas ${busy ? 'is-loading' : ''} ${onMarkerPlace ? 'marker-placeable' : ''}`}>
      ...
      {!sweep ? <div className="plot-empty"><div className="empty-atom"><AtomicMark size={76}/></div><strong>The field is quiet</strong><p>Connect an instrument and acquire a sweep.</p></div> : <svg viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none" aria-label="Measured power by frequency" onPointerDown={pointerFrequency} onPointerMove={(event) => { if (event.buttons === 1) pointerFrequency(event); }}>
        <defs><linearGradient id="trace-fill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#79f2c3" stopOpacity=".2"/><stop offset=".65" stopColor="#65dce3" stopOpacity=".045"/><stop offset="1" stopColor="#8d7cff" stopOpacity="0"/></linearGradient><filter id="trace-glow"><feGaussianBlur stdDeviation="2.1" result="blur"/><feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge></filter></defs>
