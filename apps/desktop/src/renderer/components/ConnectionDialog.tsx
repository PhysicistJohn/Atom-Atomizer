...
export function ConnectionDialog({ ports, selectedId, busy, error, onSelect, onRefresh, onConnect, onDisconnect, connected, onClose }: {
  ...
  return <div className="dialog-backdrop" role="presentation" onMouseDown={(e) => e.target === e.currentTarget && onClose()}><section className="connection-dialog" role="dialog" aria-modal="true" aria-labelledby="connection-title">
    ...
    {connected ? <div className="connected-state"><div className="connected-glyph"><Check size={24}/></div><h3>Connected</h3><button data-agent-control="connection.disconnect" className="danger-outline" onClick={onDisconnect}>Disconnect</button></div> : <>
      <div className="dialog-toolbar"><p>Available instruments</p><button data-agent-control="connection.refresh" className="text-button" onClick={onRefresh} disabled={busy}><RefreshCw size={13}/>Refresh</button></div>
      <div className="port-list">{ports.length === 0 ? <div className="no-ports"><Usb size={22}/><strong>No instrument backend found</strong><span>Connect a data-capable USB cable. The executable twin must also be available in the sibling Firmware repository.</span></div> : ports.map((port) => {
        ...
        return <button key={port.id} data-agent-control={`connection.candidate.${ports.indexOf(port) + 1}.select`} className={`port-option ${selectedId === port.id ? 'selected' : ''}`} onClick={() => onSelect(port.id)}><span className="port-icon">{twin ? <Cpu size={17}/> : <Cable size={17}/>}</span><span><strong>{port.product ?? port.manufacturer ?? (twin ? 'ZS407 executable digital twin' : 'Serial device')}</strong><small>{twin ? 'Pinned firmware · Renode bridge · USB transactions not modeled' : `${port.path}${port.serialNumber ? ` · ${port.serialNumber}` : ''}`}</small></span><i>{selectedId === port.id && <Check size={15}/>}</i></button>;
      })}</div>
      {error && <div className="inline-error">{error}</div>}
      <div className="dialog-actions"><button data-agent-control="connection.cancel" className="secondary" onClick={onClose}>Cancel</button><button data-agent-control="connection.connect" className="primary" disabled={!ports.length || busy} onClick={onConnect}>{busy ? <><LoaderCircle className="spin" size={14}/>Connecting</> : 'Connect'}</button></div>
    </>}
  </section></div>;
}
