...
export function AtomAgentPanel({open,state,status,messages,approval,execution,onClose,onSend,onVoice,onApproval}:{
  ...
  return <aside className="atom-panel" aria-label="Atom AI copilot">
    ...
    <div className={`voice-stage ${voiceActive?'active':''}`}><div className="voice-ambient"><i/><i/><i/><button data-agent-exclusion="human-agent-boundary" onClick={onVoice} disabled={!status?.configured} aria-label={voiceActive?'End voice session':'Start voice session'}>{voiceActive?<MicOff size={20}/>:<Mic size={20}/>}</button></div><div><strong>{voiceLabel(state)}</strong>{!status?.configured && <span>OPENAI_KEY required</span>}</div></div>
    <div className="atom-messages">{messages.map(message=><div key={message.id} className={`atom-message ${message.role} ${message.status??''}`}><span>{message.role==='assistant'?<AtomicMark size={14}/>:message.role==='tool'?<Check size={12}/>:message.role==='system'?<ShieldAlert size={12}/>:null}</span><p>{message.text}</p></div>)}
      {approval&&<div className="approval-card"><div><ShieldAlert size={17}/><span><strong>Approval required</strong><small>{execution==='physical'?'PHYSICAL INSTRUMENT ACTION':execution==='firmware-digital-twin'?'DIGITAL TWIN ACTION':'EXECUTION BACKEND ACTION'}</small></span></div><p>{approval.summary}. {execution==='physical'?(approval.tool==='set_rf_output'?'Confirm only after checking the connected load and attenuation.':'The firmware UI may expose physical RF-output controls; verify screen state before proceeding.'):execution==='firmware-digital-twin'?'This action changes executable firmware state but cannot radiate through the Renode bridge.':'The target is not verified as physical hardware or the executable Firmware twin.'}</p><div><button data-agent-exclusion="human-approval-boundary" onClick={()=>onApproval(false)}>Deny</button><button data-agent-control="atom.approve-high-impact" data-agent-risk="high-impact" data-agent-exclusion="human-approval-boundary" className="approve" onClick={()=>onApproval(true)}>Approve action</button></div></div>}
      <div ref={end}/>
    </div>
