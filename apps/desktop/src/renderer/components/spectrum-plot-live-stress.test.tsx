...
describe('SpectrumPlot sustained live rendering', () => {
  it('reconciles varied narrow, multipeak, and wideband frames without stale geometry or DOM growth', () => {
    ...
    const originalSvg = view.container.querySelector('svg[aria-label="Measured power by frequency"]');
    ...
    for (let update = 0; update < 210; update++) {
      ...
      const svg = view.container.querySelector('svg[aria-label="Measured power by frequency"]');
      const trace = svg?.querySelector('.trace-line');
      const points = trace?.getAttribute('points') ?? '';
      expect(svg).toBe(originalSvg);
      expect(svg?.querySelectorAll('.trace-line')).toHaveLength(1);
      expect(svg?.querySelectorAll('polygon')).toHaveLength(1);
      expect(points.split(' ')).toHaveLength(fixture.frame.frequencyHz.length);
      expect(points).not.toBe(priorPoints);
      expectFiniteSvgGeometry(svg as SVGElement);
  ...
  it('quarantines malformed updates, removes stale geometry, and recovers on the next valid frame', () => {
    ...
    for (const frame of defects) {
      ...
      expectFiniteSvgGeometry(view.container.querySelector('svg[aria-label="Measured power by frequency"]') as SVGElement);
    }
  });
  ...
  it('clips wider physical grids to the visible sweep instead of emitting off-viewport coordinates', () => {
    ...
    const tracePoints = view.container.querySelector('.trace-line')?.getAttribute('points')?.split(' ') ?? [];
    const polygonPoints = view.container.querySelector('polygon')?.getAttribute('points')?.split(' ') ?? [];

    expect(tracePoints).toHaveLength(5);
    for (const point of [...tracePoints, ...polygonPoints]) {
  ...
  it('uses instance-local SVG resources when more than one live plot is mounted', () => {
    ...
    const plots = [...view.container.querySelectorAll('svg[aria-label="Measured power by frequency"]')];
    expect(plots).toHaveLength(2);
    const gradientIds = plots.map((plot) => plot.querySelector('linearGradient')?.id);
    const filterIds = plots.map((plot) => plot.querySelector('filter')?.id);

    expect(new Set(gradientIds).size).toBe(2);
    expect(new Set(filterIds).size).toBe(2);
    plots.forEach((plot, index) => {
      expect(plot.querySelector('polygon')?.getAttribute('fill')).toBe(`url(#${gradientIds[index]})`);
      expect(plot.querySelector('.trace-line')?.getAttribute('filter')).toBe(`url(#${filterIds[index]})`);
    });
  });
  ...
  it('replaces marker pointer callbacks during churn instead of retaining stale handlers', () => {
    ...
    const svg = view.container.querySelector('svg[aria-label="Measured power by frequency"]');
    if (!(svg instanceof SVGSVGElement)) throw new Error('Expected live spectrum SVG');
    vi.spyOn(svg, 'getBoundingClientRect').mockReturnValue({
      left: 0, right: 1_200, top: 0, bottom: 430, width: 1_200, height: 430, x: 0, y: 0, toJSON: () => ({}),
    });

    fireEvent.pointerDown(svg, { clientX: 600, clientY: 200 });
