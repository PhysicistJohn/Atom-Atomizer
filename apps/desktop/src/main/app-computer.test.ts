...
describe('app-scoped computer harness', () => {
  ...
  it('blocks click, type, key, and scroll at local human-only boundaries', async () => {
    ...
    expect(await harness.click(win as never, clickShot.screenshotId, 10, 20)).toMatchObject({ ok: false, target: 'firmware.flash' });
    webContents.executeJavaScript.mockResolvedValue({ target: 'firmware.flash', editable: true, blockedReason: 'local human-only boundary' });
    expect(await harness.type(win as never, 'firmware.flash', 'FLASH')).toMatchObject({ ok: false, target: 'firmware.flash' });
    expect(await harness.key(win as never, 'firmware.flash', 'ENTER')).toMatchObject({ ok: false, target: 'firmware.flash' });
    const scrollShot = await screenshot(harness, win);
    webContents.executeJavaScript.mockResolvedValueOnce({ target: 'firmware.flash', blockedReason: 'local human-only boundary' });
    expect(await harness.scroll(win as never, scrollShot.screenshotId, 10, 20, 0, 100)).toMatchObject({ ok: false, target: 'firmware.flash' });
