...
TinySA capabilities are connection evidence, not a static OEM profile. After identity and mandatory RF-off/readback commands succeed, `TinySaDeviceService` derives `DeviceCapabilities` from that session and the driver projects only those capabilities into the generic instrument contract. Source-qualified OEM, executable-twin, and protocol-fixture revisions may use the pinned ZS407 ranges, but optional commands still gate optional acquisitions and features. A `custom-unqualified` revision receives no OEM range defaults: receiver control ranges and automatic modes come from parseable command usage, frequency and point bounds are conservatively limited to the connected device's observed geometry, and enum controls come only from advertised syntax. Leveled triggers are withheld without an advertised threshold range, and the generator is withheld because a command name alone cannot prove its safe frequency/level domain. Unknown or malformed ranges fail closed. A custom surface that cannot form at least one complete acquisition is rejected and its pre-session transport is cleaned up.
