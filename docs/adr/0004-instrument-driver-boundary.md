...
- Amended: 2026-07-31 for the registered NeptuneSDR/HAMGEEK P210 driver (see the dated update near the end of this document)
- Amended: 2026-08-01 for the SignalLab v3 unbounded long-dwell bridge and trio composition v7

## Context

Atomizer originally composed `TinySaDeviceService` directly in Electron main. That made the physical ZS407 protocol, application startup, and operator UI one implicit device contract. It also encouraged a synthetic source to masquerade as a serial port or firmware device. SignalLab is now a live high-level measurement source with synthetic scalar measurements and bounded deterministic complex I/Q for all 44 closed profiles: 31 content-addressed fixed digital artifacts, two unbounded Bluetooth long-dwell engineering compositions, eight analytic lab/reference generators, and three custom standards builders. NeptuneSDR P210 is separately registered with materially different hardware identity, configuration, native formats, and bounded complete-buffer semantics. The application needs one boundary that admits present and future sources without erasing what each one can honestly prove.
...
Source provenance also constrains capability truth in v1. SignalLab may expose synthetic scalar spectrum/detected-power acquisition, profile/channel selection, and bounded deterministic complex I/Q for all 44 explicitly advertised closed profiles. Eight lab/reference generators retain `analytic-complex-baseband`; three custom builders and two unbounded Bluetooth long-dwell compositions retain `standards-derived-complex-baseband`; 31 fixed artifacts distinguish exact native `independently-verified-digital-baseband`, clean `derived-from-independently-verified-digital-baseband`, and explicit receiver-impaired output. The unbounded compositions have native-rate geometry but no canonical artifact, cyclic period, or terminal capture bound; derived FIR support zero-extends only before session origin. RF generator, screen, touch, diagnostics, and receiver controls remain rejected at session admission. Serial and firmware-twin scalar acquisitions require receiver controls. Export admission mirrors this binding, binds the complete requested closed/half-open frequency grid to samples, and uses generic instrument ceilings rather than TinySA frequency limits. JSON preserves the full admitted request, and CSV carries it in `requested_configuration_json`.

Instrument v1's source-provenance union is closed to `serial-port`,
`tinysa-firmware-twin`, `signal-lab`, `neptune-p210`, and `neptune-p210-twin`.
The renderer has capability-gated
scalar and complete-buffer I/Q surfaces, while scalar staging remains TinySA-shaped. NeptuneSDR did not borrow serial-port or any
other existing provenance merely because its transport is a network context.
It got a truthful first-class transport/provenance variant of its own
(`neptuneP210`/`neptuneP210Twin`, `transport: 'libiio-network'`) and its own
hardware-specific export path, exactly as this section required; it was not
smuggled through a false existing source or projected into scalar controls.

The paired SignalLab measurement bridge v3 is active. Its exact in-process schema includes per-profile governance, native/output I/Q geometry, continuous/cyclic/one-shot/unbounded replay, explicit receiver impairment, payload SHA-256, and transform receipts, plus a required safe-integer detected-power center on an advertised 1 Hz tuning lattice. The producer contract, catalog, and domain-separated contract binding are hash-bound; the binding is explicitly not generator-code identity. Older strict Atomizer builds reject a mismatched producer before dispatch. Any wire-field or semantic change requires a new bridge contract version rather than mutation of the frozen v2 or v3 documents.

## Capability growth and NeptuneSDR

The base contract contains common acquisition variants, swept scalar spectrum, uniformly sampled detected power, and a complete single-buffer complex-I/Q shape capped at 64 MiB, plus narrowly typed optional features. Continuous complex-IQ acquisition is rejected; chunking, continuation, backpressure, or long-lived streaming requires a new contract version. SignalLab advertises bounded complete-buffer `cf32le` for all 44 closed profiles, including exact per-profile geometry, transport derivation, replay bounds, and qualification; its two unbounded long-dwell profiles use the existing session-origin zero-extension policy rather than a streaming claim. `neptune-p210` advertises exactly one bounded complete-buffer `ci16le` complex-I/Q capability (receive-only, up to 8,388,608 samples) for both its `neptune-p210` and `neptune-p210-twin` source kinds and nothing else; `tinysa-zs407` advertises no complex-I/Q at all. The UI and agent surface derive availability from those declarations.

New common semantics require a versioned contract addition with manager validation and at least two credible consumers; they are not placed in an untyped options bag. A truly source-specific operation would require a driver-owned, separately versioned extension contract and would remain unavailable to other drivers; instrument v1 has no generic extension hook. NeptuneSDR/HAMGEEK P210 is now registered as driver `neptune-p210`, added without any such source-specific extension contract — its complex-I/Q acquisition, ADC evidence fields, and provenance all fit inside the common instrument v1 shapes. The transport-neutral runtime package solved only driver lifecycle coupling; it did not make source registration open-ended, and the addition still required a distinct driver/source identity, truthful hardware capabilities and provenance, native-format export and configuration paths (delivered as SigMF), consumers (UI and the minimal `acquire_complex_iq` Atom tool), contract tests, pre-session lease cleanup through the required driver hook, and the coordinated trio-contract v6 initial revision followed by v7 — all now in place. Long-lived I/Q still requires a versioned streaming contract for chunking, ordering, backpressure, cancellation, and bounded retention; the existing complete single-buffer I/Q value and renderer remain not a streaming protocol, and Neptune v1 supplies no continuous-acquisition path.
...
- Trio composition v7 records the active in-process SignalLab v3 measurement edge, selectable TinySA physical/twin sources, the registered `neptune-p210` driver, factory-default/no-fallback rule, reserved SignalLab stimulus edge, and Atom-Flasher exclusive-ownership boundary; v6 remains frozen evidence of the Neptune delivery.

## 2026-07-31 update: NeptuneSDR/HAMGEEK P210 registered

NeptuneSDR/HAMGEEK P210 was added as driver `neptune-p210` (source kinds `neptune-p210` physical, `neptune-p210-twin` QEMU twin), with its own first-class candidate/provenance schemas, a single receive-only `complex-iq` acquisition capability, byte-exact SigMF export, and capability-gated Atom acquisition/export tools, initially recorded in trio composition v6 and retained in active v7. Discovery now combines optional bootstrap endpoints, libiio Bonjour/bounded local scanning, a manual-once UI path, and direct recent-device re-probing. The physical `ip:10.0.0.250` path passed live manager capture plus product-layer retune/readback, Single/Run/Stop, and relative derived-view acceptance on 2026-07-31. One-at-a-time backpressured buffers provide bounded Run behavior, but no driver-native chunk/overrun protocol is claimed. Calibrated power, RF generation, screen/touch/diagnostics, multi-channel/MIMO capture, full-range RF qualification, bundled cross-platform libiio, and QEMU-twin acceptance remain future work. See [../NEPTUNE_P210_CHARACTERIZATION.md](../NEPTUNE_P210_CHARACTERIZATION.md).
