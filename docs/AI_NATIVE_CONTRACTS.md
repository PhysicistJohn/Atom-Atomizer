...
SignalLab Studio profile/channel controls and complete-buffer I/Q capture are currently capability-gated, human-operated UI surfaces. They do not silently become Atom tools: semantic control and coordinate computer use remain excluded until a typed schema, evidence projection, policy, executor, and tests are admitted. The generic I/Q contract and renderer do not authorize an agent operation.
...
- **AI-55:** Embedded SignalLab profile/channel controls and complete-buffer I/Q capture remain capability-gated UI-only exclusions until each receives an explicit typed Atom contract; Atom must preserve the exact analytic/custom/native-qualified/derived/impaired label and may not upgrade family labels or digital-baseband qualification into packet identity, RF/antenna, regulatory, interoperability, or product-conformance evidence.
