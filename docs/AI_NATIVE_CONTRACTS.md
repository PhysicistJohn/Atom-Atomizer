...
Atomizer owns operator intent, instrument selection/lifecycle, OpenAI credentials, tool policy, approvals, and orchestration. `Atom-SignalLab` owns the active high-level synthetic measurement producer and the separate future stimulus intent. `Atom-Firmware` owns the executable twin and that future intent's sink. Atom reads topology when source identity or composition is material and preserves these distinctions:

| Source | Driver / transport | Qualification | USB status |
|---|---|---|---|
| SignalLab | `signal-lab` / in-process `signal-lab-measurement-bridge-v3` | Scalar `synthetic-visual-projection`; all-profile deterministic I/Q: eight analytic lab/reference profiles, three custom engineering builders, two unbounded Bluetooth long-dwell compositions, and 31 content-addressed fixed digital artifacts with exact-native or derived lineage; every impaired result is explicitly downgraded; no RF, firmware, generator, screen, or touch claim | Not a USB source |
...
The byte-identical trio composition v7 manifest is normative. With no persisted preference, the exact `signal-lab:default` candidate is the factory default; a failed or ambiguous preference never falls back to a different source. Atom may observe and operate only capabilities declared by the active `AtomizerInstrumentApiV1` session. SignalLab's selected profile is status/capability state and cannot become detector or classifier evidence. All 44 closed profiles have admitted deterministic complex-I/Q. For the 31 fixed profiles, only exact clean native bytes retain `independently-verified-digital-baseband`; clean transport transforms retain `derived-from-independently-verified-digital-baseband`; the two unbounded Bluetooth long-dwell compositions have no canonical artifact and remain `standards-derived-complex-baseband`; and receiver impairment is an explicit downgrade. These are digital-byte and lineage claims, not RF, antenna, product, or regulatory claims. Atom cannot infer protocol identity or RF behavior from a family tab or generated buffer. The active SignalLab→Atomizer and Neptune→Atomizer edges remain distinct from the SignalLab→Firmware stimulus edge, which is `reserved-not-connected`; Atom cannot activate that edge or describe the twin as USB.

Source-specific profile, channel, replay, and impairment details remain inside their driver. Atom reads the current canonical surface and can request only its declared generic operations through `execute_canonical_operation`; it never receives a fixed source-family tool or native setting schema. Complete-buffer I/Q capture is likewise capability- and admitted-configuration-gated through the shared acquisition tools. Driver-specific labels and values remain evidence, not protocol, RF, antenna, regulatory, interoperability, or product-conformance claims.

NeptuneSDR P210 is a registered receive-only driver with versioned physical/twin identity, truthful libiio provenance, bounded `ci16le` I/Q, manager validation, dBFS-relative derived-spectrum consumers, and trio composition v7. Atom may offer the capability-gated `acquire_complex_iq` and `export_latest_iq` tools and the shared bounded Single/Run controls. It must not infer calibrated dBm, transmit authority, screen/touch/diagnostics, MIMO, or a driver-native chunk/overrun stream because the active session advertises none of them. Physical `ip:10.0.0.250` has live receive-path evidence; QEMU-twin and RF/metrology claims remain unqualified.
...
| Connecting | Voice animation and cancel | Cancel stops immediately; an explicit typed request stops voice startup and continues as text without dropping the prompt |
...
1. When `OPENAI_KEY` is configured, renderer makes one automatic startup connection attempt with microphone state already muted. The complete attempt is bounded to 15 seconds, including microphone capture and WebRTC setup; failure is visible and is not retried automatically.
...
- A renderer-global ownership lease and synchronous startup token permit only one peer/session construction at a time, covering auto-connect/manual-connect, React teardown, hot-module replacement, and in-flight media-permission races. A canceled or timed-out asynchronous start cannot resurrect a peer. An explicit typed request preempts voice startup or an active voice session, then proceeds through the text transport instead of being discarded. A second remote audio track or distinct stream terminates the session rather than producing double playback; one connected session still preserves normal VAD barge-in.
...
11. Loop ends on an assistant response, explicit user denial, transport/protocol failure, or the eight-application-call bound. An ordinary failed application result remains available for bounded recovery with an already selected tool.

The loader is accepted once per submitted user operation; a tool failure may be
handled only with tools that operation already selected. Loading a schema is
not executing its tool. A peak-marker request therefore selects
`search_marker` and `acquire_sweep` together, searches an existing complete
assigned trace without reacquiring, and uses the preloaded acquisition only to
recover from no trace data or to satisfy an explicit request for fresh evidence.
When both calls are needed, the renderer preserves emitted order and completes
the sweep before marker search.
...
| `get_application_state` | Observe | Never | Reads route/acquisition/environment, current driver-admitted configuration, canonical operation surface, and host measurement state |
| `get_system_topology` | Observe | Never | Reads the versioned driver host and active transport/execution/evidence topology without turning a driver detail into a fixed tool |
...
| `get_instrument_state` | Observe | Never | Reads generic session identity, qualification, capabilities, admitted configuration, canonical surface, and driver-reported output state |
...
| `acquire_envelope_stft` | Operate | Never | Acquires admitted detected-power evidence and returns its Not-I/Q STFT |
...
| `acquire_sweep` | Operate | Never | Runs Single against the current compatible driver-admitted spectrum or complex-I/Q configuration; missing configuration fails with a canonical-operation prerequisite |
| `start_continuous_sweeps` | Operate | Never | Runs against the current compatible driver-admitted spectrum or complex-I/Q configuration; missing configuration fails with a canonical-operation prerequisite |
...
| `acquire_zero_span` | Operate | Never | Captures and characterizes one envelope using the current admitted detected-power configuration |
| `acquire_complex_iq` | Operate | Never | Captures one complete bounded complex-I/Q buffer using the current admitted complex-I/Q configuration; unavailable capability or configuration fails closed |
| `execute_canonical_operation` | Operate / high impact when declared | At action when declared high impact | Sends one complete Auto/Manual intent set for one current driver-declared operation; native translation, effective values, verification, and output safety remain driver-owned |
...
| `export_latest_iq` | Operate | Never | Opens a native save dialog for byte-exact SigMF metadata and sample data bound to the active session |
...
All 48 registered parameter schemas are generated from the same Zod objects used to accept execution. The persistent Realtime session never carries that entire registry: its one loader schema contains only the closed name enum, and `response.create` installs the selected concrete definitions for one response. Realtime requires a closed top-level `type: object` and rejects top-level `oneOf`, `anyOf`, `allOf`, `enum`, `const`, and `not`; catalog tests enforce that admission rule for every application tool. Cross-field constraints (marker delta reference, channel overlap, waterfall floor/ceiling, STFT hop/window, and each driver's canonical operation domain) remain fail-closed runtime refinements and are repeated in tool/property descriptions.

Transient numeric-entry panels are body-level portals carrying the originating row identity in `data-parameter-editor`. While open, the one stable `data-agent-control` moves from the occluded source row to the portal and the source becomes an explicit exclusion, so duplicate hooks cannot appear. Their field, keypad, unit terminators, and close/apply controls cannot create a second configuration path. When a domain tool exists, Atom uses it instead of reproducing keypad clicks; instrument configuration uses `execute_canonical_operation` and host projection state uses its corresponding typed tool. Computer operation remains an app-scoped semantic/visual path, not a second validation path.
...
`custom-source-qualified-receive-only` identity maps one exact embedded version
to one frozen audited source commit for narrowly declared receiver behavior.
Atom must preserve its warning that the serial identity does not attest the
documented binary SHA-256 and must not call it OEM, hardware/RF, or metrology
qualification. Its missing RF-generator feature and `rfOutput=not-supported`
grant no RF-output authority; internal output-off reassertion is safety behavior,
not an Atom feature.
...
- Firmware identity remains observable device evidence but cannot create an Atomizer installation capability; users must use the standalone `Atom-Flasher`.
...
- **AI-42:** Every one of the 48 tool definitions has a concrete JSON schema, matching runtime validator and policy; instrument mutation is one complete current driver-declared canonical operation.
...
- **AI-55:** Driver-specific source details enter Atom only through the active driver's canonical surface and evidence projection; no fixed source-family tool or UI route exists. Atom must preserve analytic/custom/native-qualified/derived/impaired qualification and may not upgrade it into packet identity, RF/antenna, regulatory, interoperability, or product-conformance evidence.
- **AI-56:** Any action-containing response batch receives complete schema/authorization preflight and fails fast after its first execution failure or approval denial, with exactly one actual failed, actual cleanup, or recoverable-skipped result per emitted call; only RF-off, stop-streaming, and disconnect may cross a barrier as best-effort safety cleanup, while all-observe batches retain independent-read semantics and high-impact approval remains bound to a just-in-time complete backend identity.
