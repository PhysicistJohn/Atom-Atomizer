...
- SignalLab admits all 44 closed profile IDs. Each capability row distinguishes
...
label, availability, confirmation level, parameter domains, recommendation
policy, effective values, verification evidence, and any cross-setting
constraints. Atomizer may choose placement and generic layout only; it never
...
- **IQ-06:** SignalLab exposes deterministic I/Q for all 44 closed profiles, preserves analytic/custom/unbounded/native-qualified/derived/impaired qualification and exact transform lineage, and never upgrades digital-baseband evidence into packet identity, RF/antenna, regulatory, or product-conformance evidence.
