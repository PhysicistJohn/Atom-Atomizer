...
| NeptuneSDR P210 (v1) | registered receive-only `neptune-p210` physical/twin driver; bounded `ci16le` I/Q; optional-env, Bonjour/local-scan, manual-once and recent-device discovery; GUI-path libiio resolution; byte-exact SigMF UI/Atom export; host-derived dBFS-relative Spectrum/Waterfall/Channel/Detect/traces/markers; one-at-a-time backpressured Run; trio contract v7; physical `ip:10.0.0.250` passed driver/manager and product-layer connect, retune/readback, Single/Run/Stop, derived-view acceptance | calibrated power, RF generation, screen/touch/diagnostics, driver-native chunk streaming/overrun semantics, multi-channel/2x2 MIMO, maximum-range soak, known-signal RF characterization, bundled cross-platform libiio, and QEMU-twin acceptance; see [docs/NEPTUNE_P210_CHARACTERIZATION.md](./docs/NEPTUNE_P210_CHARACTERIZATION.md) |
