...
| Contracts | strict API v2, Atom surface v5, byte-identical trio composition v2, physical/OEM firmware provenance, updater, device/sweep/zero-span/screen/diagnostics/export/analysis/measurement contracts | operation IDs and schema migrations before public file persistence |
...
Voice call creation sends only the immutable `{ type: "realtime", model: "gpt-realtime-2.1" }` bootstrap with SDP. Before enabling the muted microphone, the data channel sends and verifies the complete session: `audio.output.voice: ballad`, `audio.input.turn_detection` as `server_vad` with threshold `0.97`, automatic response creation/interruption, `gpt-realtime-whisper` transcription, high reasoning, instructions, and all 53 tools. Chromium requests echo cancellation, noise suppression, and automatic gain control. Requested/applied microphone settings and sent/API-returned Realtime session settings are emitted to the console.
