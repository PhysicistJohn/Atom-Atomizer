...
SignalLab is an independent repository, application, and active high-level measurement producer. Atomizer bundles its platform-neutral `AtomizerMeasurementService` and version-1 measurement contract directly into the desktop main process and web edition; there is no measurement subprocess or packaged sibling-resource lookup. The service provides synthetic swept-spectrum and detected-power observations plus normalized deterministic complex-I/Q through the `signal-lab` driver. Clean and seeded receiver-impaired I/Q remain simulation-native engineering evidence, not packet-decodable or conformance vectors. A request below a wideband profile's catalogued occupied support returns a disclosed deterministic discrete-time alias projection rather than an alias-free full-channel reconstruction. SignalLab claims no USB emulation, firmware execution, RF emission, generator, display, or touch capability; its I/Q is explicitly simulation-native rather than hardware identity. Each detected-power request carries one required safe-integer center frequency on the advertised 1 Hz lattice; the source returns that exact tune and receiver-filters its source model there. SignalLab owns the waveform descriptors, deterministic AWGN/Rayleigh scalar channel configuration, the closed receiver-I/Q presets (AWGN, multipath, carrier offset, phase noise, I/Q imbalance, DC offset, PA compression, and composite stress), and `SignalLabStimulusIntent`. Named standards models are excluded from selection when the renderer does not reproduce their required allocation, PRB sequence, timing, or spectral partition. Selected profile and channel configuration are status/capability state, never classifier evidence. Its separate Firmware stimulus sink remains `reserved-not-connected`; activating that future edge requires a coordinated contract-version change in all three repositories.
...
| SignalLab measurements | `Atom-SignalLab` contract v1 | `signal-lab` driver | Active bounded in-process edge; synthetic high-level measurements, never USB/firmware/RF evidence |
