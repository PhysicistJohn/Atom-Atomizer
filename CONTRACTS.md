...
Status: active execution baseline, instrument API v1, SignalLab measurement bridge v3, TinySA protocol v3, trio composition v7, 2026-08-01
Companion: [PLAN.md](./PLAN.md)

Trio authority: [contracts/trio-composition-v7.json](./contracts/trio-composition-v7.json)
...
- Current transports are the SignalLab versioned in-process service, physical USB CDC serial, and the Renode monitor bridge. Each retains a distinct source kind, execution class, qualification, and provenance. Firmware update and DFU are absent from Atomizer and owned exclusively by the standalone sibling `../Atom-Flasher`; raw/generic DFU remains excluded here.
...
The current repository has accepted automated evidence for `AtomizerInstrumentApiV1`, the driver registry/manager boundary, SignalLab producer-consumer interoperation, exact TinySA prompt/parser/scheduler behavior, physical serial and Renode bridge boundaries, Neptune P210 network discovery/configure/capture/projection safety, analyzer text/raw/zero-span acquisition, bounded complete-buffer complex-I/Q acquisition and visualization, diagnostics, screen/touch, generator safety sequencing, persistent detection, bounded classification, advanced scalar-sweep measurements, export serialization, Atom surface v11, five legacy `WorkspaceId` values, four rendered workspace surfaces, and six visible sidebar destinations. The legacy Detection and Classification route values converge on the one visible Detect surface; Spectrum, Waterfall, and Channel select the one measurement surface without a nested view-tab bar. Firmware installation is intentionally absent. The initial physical receive-only slice is accepted as recorded evidence, not general RF-hardware qualification. Generated embedding-classifier assets are optional application payloads, never a desktop startup prerequisite. If they are absent or rejected by the runtime contract, only local modulation classification becomes explicitly unavailable/unknown; connection, acquisition, detection, Spectrum/Waterfall/Channel, Generate, and Device continue under their own capability gates. The Bayesian scalar-observable pipeline remains separate research and regression infrastructure rather than the rendered production classifier.

Atomizer statically registers `signal-lab`, `tinysa-zs407`, and `neptune-p210`. It discovers each driver independently, retains driver-scoped failures, and connects exactly one candidate selected by the owner-only version-1 driver/source/candidate preference. New writes bind the exact candidate ID; legacy v1 preferences without that field remain readable but fail closed when broad matching is ambiguous. If no preference file exists, exact candidate `signal-lab:default` is the explicit factory default. A corrupt preference, zero or multiple preferred matches, discovery failure, bridge failure, TinySA identity failure, Neptune endpoint/capture failure, or evidence mismatch is visible and never activates another driver or source kind. An operator may explicitly select and persist SignalLab, a physical ZS407, the executable twin, or a Neptune P210 physical/twin source after safe disconnection.

SignalLab is an independent repository, application, and active high-level measurement producer. Atomizer bundles its platform-neutral `AtomizerMeasurementService` and strict version-3 measurement contract directly into the desktop main process and web edition; there is no measurement subprocess or packaged sibling-resource lookup. The service provides synthetic swept-spectrum and detected-power observations plus deterministic complex-I/Q through the `signal-lab` driver for all 44 profiles. Thirty-one fixed profiles bind content-addressed independently verified digital-baseband artifacts to exact native geometry and cyclic or one-shot replay policy; two Bluetooth long-dwell engineering compositions use an unbounded native 80 MHz timeline without a canonical artifact or terminal capture bound; eight lab/reference profiles and three custom builders are rate-flexible. Exact clean native bytes, clean derived transport, analytic generation, standards-derived engineering generation, and receiver-impaired output retain distinct qualifications. Every result separates signal bandwidth from capture bandwidth and RF reference/profile center/carrier offset/tune, binds source and output windows to an explicit transform receipt, and carries a SHA-256 that Atomizer recomputes over the delivered bytes. These are digital-baseband byte and lineage claims, not packet identity, RF emission, antenna performance, regulatory approval, or product certification. SignalLab claims no USB emulation, firmware execution, RF emission, generator, display, or touch capability. Each detected-power request carries one required safe-integer center frequency on the advertised 1 Hz lattice; the source returns that exact tune and receiver-filters its scalar source model there. SignalLab owns the waveform descriptors and per-profile governance, deterministic AWGN/Rayleigh scalar channel configuration, the closed receiver-I/Q presets (AWGN, multipath, carrier offset, phase noise, I/Q imbalance, DC offset, PA compression, and composite stress), and `SignalLabStimulusIntent`. Selected profile and channel configuration are status/capability state, never classifier evidence. Its separate Firmware stimulus sink remains `reserved-not-connected`; activating that future edge requires a coordinated contract-version change in all three repositories.
...
| SignalLab measurements | `Atom-SignalLab` measurement bridge v3 | `signal-lab` driver | Active bounded in-process edge; strict profile governance, scalar projections, and digitally qualified or explicitly derived/impaired I/Q; never USB/firmware/RF evidence |
...
| Neptune P210 | Device / Atomizer driver | `neptune-p210` driver | Receive-only libiio network I/Q; physical device-observed or distinct QEMU-twin provenance; no scalar/TX/calibrated-power claims |
...
commands; and in-process SignalLab results are bounded by strict producer and
consumer schemas with no independent input queue. Overflow is visible failure
and never crowds out output-off/disconnect or activates another source.
Physical serial enumeration/open/close are bounded, admit
...
durable Atomizer↔Flasher handoff lease: composition v6 requires a local-human
...
the in-process SignalLab driver owns no external transport or child process but
still supplies the required idempotent cleanup hook.

### Assume/guarantee composition rule

For each active producer→consumer edge, release composition is valid only when the producer guarantee implies every consumer assumption: `G_producer => A_consumer`. The three runtime repositories carry byte-identical copies of composition contract v7; `npm run check:firmware-twin` validates the active copy's version, `$id`, and firmware-party pins at runtime.

- SignalLab→Atomizer assumes the exact version-3 contract, catalog hash, and contract-binding hash. SignalLab guarantees bounded in-process execution, high-level synthetic spectrum/detected-power observations, and bounded deterministic complex-I/Q for all 44 closed profiles. Thirty-one fixed artifacts have content-addressed qualification, native geometry, and replay bounds; two unbounded Bluetooth long-dwell compositions have native 80 MHz geometry but no canonical artifact or terminal capture bound; clean transformed output retains explicit derived lineage; eight analytic lab/reference and three custom builder profiles retain their narrower qualifications; every impairment is an explicit downgrade. Every payload and exact transform receipt is hash-bound, integer-Hz detected-power tuning is source-filtered and returned exactly, USB-emulation/firmware-execution/RF-emission claims are false, and selected profile remains excluded from measurement evidence. Digital-baseband qualification does not claim RF, antenna, regulatory, interoperability, or product conformance.
...
Exact candidate `signal-lab:default` is the factory default only when no preference file exists. The physical ZS407 and Firmware twin are selectable source kinds owned by `tinysa-zs407`; availability never authorizes fallback. NeptuneSDR/HAMGEEK P210 is registered as driver `neptune-p210`, declaring source kinds `neptune-p210` (physical board) and `neptune-p210-twin` (QEMU digital twin), with its own truthful hardware provenance and a single `complex-iq` acquisition capability (`ci16le`, receive-only, no RF generator, no native scalar acquisition). It is neither the factory default nor selectable through automatic fallback: manual operator selection or a persisted `neptune-p210` preference is required. Discovery honors optional endpoint environment variables, probes libiio Bonjour and bounded directly attached subnets, and re-probes driver-owned recent-device records; a routed endpoint can be verified once through the source dialog and then rediscovered without a shell environment. `neptune-p210` supplies the required idempotent `cleanupPendingConnection()` hook and participates in the aggregate manager/quit teardown gate exactly like `signal-lab` and `tinysa-zs407`. Native-format export exists (SigMF `.sigmf-meta`/`.sigmf-data` via UI and `export_latest_iq`), and Atom's `acquire_complex_iq` tool exercises the same admitted-configuration capture path used by SignalLab. Each accepted I/Q buffer may project one instantaneous scalar spectrum with explicit `uncalibrated-dbfs-relative` reference; this populates relative Spectrum/Waterfall/Channel/Detect/trace/marker analyses without inventing dBm. Run schedules bounded buffers one at a time with backpressure; it is not a driver-native chunk/overrun streaming protocol. The physical `ip:10.0.0.250` path passed live driver/manager capture and product-layer Single/Run/Stop, retune/readback, and derived-view acceptance on 2026-07-31. Still absent: calibrated power, transmit/generator authority, screen/touch/diagnostics, native chunk-stream semantics, multi-channel/2x2 MIMO, full-range RF characterization, bundled cross-platform libiio, and QEMU-twin acceptance. See [docs/NEPTUNE_P210_CHARACTERIZATION.md](./docs/NEPTUNE_P210_CHARACTERIZATION.md).
...
Excluded from Atomizer unless added by change order: DFU/firmware flashing and custom-firmware installation (both already owned exclusively by sibling `../Atom-Flasher`, not Atomizer change packages); calibration writes; factory reset; unrestricted raw console; multiple simultaneous devices; remote/network access; cloud sync; accounts; mobile/web apps; automated regulatory/compliance measurements; control of other tinySA/NanoVNA models; third-party executable plug-ins; localization; app-store submission. An internal typed analysis-mode extension interface is included. NeptuneSDR/HAMGEEK P210 is an implemented driver/source (`neptune-p210`, receive-only complex-I/Q v1) with bounded one-at-a-time product Run behavior and manual/recent/best-effort scan discovery, but calibrated power, RF generation, screen/touch/diagnostics, driver-native chunk/overrun streaming, multi-channel/MIMO capture, and RF metrology remain explicitly excluded.
...
- **CP-04 Additional drivers/models:** NeptuneSDR/HAMGEEK P210 v1 (`neptune-p210`) is delivered — distinct driver/source identity, provenance/capability profiles, optional-env plus manual/recent/best-effort scan discovery, contract tests, I/Q and relative derived-spectrum UI/Atom consumers, SigMF export, cleanup lifecycle, trio contract v7, and live physical receive-path acceptance are in place. Remaining follow-on work: bundled cross-platform libiio, calibrated RF characterization, a versioned driver-native chunk/overrun protocol, multi-channel support, and QEMU-twin acceptance (see [docs/NEPTUNE_P210_CHARACTERIZATION.md](./docs/NEPTUNE_P210_CHARACTERIZATION.md)).
