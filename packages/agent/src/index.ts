...
export const agentControlBindings: readonly AgentControlBinding[] = [
  ...
  { pattern: /^classification\.candidate\.[A-Za-z0-9-]{1,128}\.select$/, preferredTool: 'select_classification_candidate', risk: 'operate', projection: 'ui-only', guarantee: 'Selects exactly one current detected-signal result, clears any differently bound envelope, and stages its nearest capability-admitted detected-power tune without acquiring.' },
...
const agentToolDescriptors: readonly AgentToolDescriptor[] = [
  ...
  { type: 'function', name: 'get_detection_results', description: 'Read separately projected frequency-local detections, 2.4 GHz frequency-agile activity, and static classifier associations. Multicomponent swept-region associations retain every admitted per-sweep local member observation. Associations are explicitly neither physical emissions nor common-process, simultaneity, emitter, or protocol identity.' },
  { type: 'function', name: 'get_classification_results', description: 'Read open-set Bayesian observable evidence classes from repeated scalar spectra and optional qualified detected-power envelope evidence. A grouped result carries its exact current association members and multicomponent lineage. Results include a proper unknown probability and are equivalence classes, never SignalLab selected-state proof, common-process or simultaneity proof, protocol decoding, conformance, emitter identity, or I/Q classification.' },
  ...
  { type: 'function', name: 'select_classification_candidate', description: 'Select one current detected-signal ID, clear any envelope bound to another selection, and stage the nearest admitted detected-power tune without acquiring.' },
