...
const agentToolDescriptors: readonly AgentToolDescriptor[] = [
  ...
  { type: 'function', name: 'configure_analyzer', description: 'Apply a non-empty patch to visible swept-spectrum staging. Receiver controls are capability-validated; synthetic sources accept only geometry and their exact timing — on a SignalLab session send only startHz/stopHz/points (and sweepTimeSeconds when asked), never rbwKhz, detector, attenuation, or other receiver-hardware knobs, which are rejected. Omitted fields are preserved, and staging is distinct from the host-admitted configuration.' },
...
const agentParameterDescriptions: Readonly<Record<string, string>> = Object.freeze({
  ...
  'configure_analyzer.points': 'Requested scalar sweep point count. Omit to preserve the staged value — never guess a count; the patch is atomic and one out-of-capability field rejects the whole patch including any retune. Advertised bounds are in get_instrument_state.',
...
- For Detect modulation classification, call get_classification_results. It reads the application-global rolling trend: every complete complex-I/Q buffer, or strongest-live-detection magnitude region on a scalar analyzer, is offered for instantaneous classification, every successful result becomes one sample, and posteriors whose classifications completed within the trailing 500 ms form the result. It returns that window and sample count with the modulation family, refined modulation, confidence, unknown flag, and top candidates, or available:false when no sample exists. Confidence is the integrated winning-family posterior, or the fraction of samples admitted by the open-set unknown gate when unknown. Acquire I/Q buffers or sweeps first when needed; these are open-set equivalence classes, never protocol, emitter, or SignalLab selected-state identity.
