...
const EXPECTED_BINDING_PATTERNS = [
  ...
  '^analyzer\\.(start|stop|points|rbw(-mode)?|transfer|attenuation(-mode)?|sweep-time(-mode)?|detector|spur-rejection|avoid-spurs|lna|trigger|trigger-level)$',
...
const EXPECTED_CONCRETE_CONTROL_IDS = [
  ...
  'analyzer.detector', 'analyzer.lna', 'analyzer.points', 'analyzer.preset.2g4', 'analyzer.preset.5g',
  'analyzer.preset.fm', 'analyzer.rbw', 'analyzer.rbw-mode', 'analyzer.spur-rejection', 'analyzer.start',
  ...
  'connection.cancel', 'connection.close', 'connection.connect', 'connection.disconnect',
