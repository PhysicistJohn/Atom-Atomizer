...
describe('trio composition v7', () => {
  it('preserves frozen composition history while binding the v3 SignalLab bridge in every runtime copy', async () => {
    const [v4, v5, atomizerV6, signalLabV6, firmwareV6, atomizerV7, signalLabV7, firmwareV7] = await Promise.all([
    ...
    const historical = JSON.parse(atomizerV6.toString('utf8')) as {
    ...
    expect(historical.contractVersion).toBe(6);
    expect(historical.parties.atomizer.agentSurfaceVersion).toBe(11);
    const historicalNeptune = historical.edges.find((edge) =>
      edge.producer === 'neptune-p210' && edge.consumer === 'atomizer');
    expect(historicalNeptune).toMatchObject({
      status: 'active',
      transport: 'libiio-network-through-neptune-p210-driver',
    });
    expect(JSON.stringify(historicalNeptune)).toMatch(/three consecutive failures suspend/i);
